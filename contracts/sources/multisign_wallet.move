...
module surge::msign_wallet{
    ...
    struct BitcoinProposal has store {
        ...
        /// The proposal id
        proposal_id: u64,
        /// The proposal transaction id / latest psbt
        psbt: address,
    ...
    struct RoochProposal has store {
        ...
        /// The signer will sign the sign_message + psbt
        /// We keep the tx_data here for the verify the psbt and display to the user
        tx_data: vector<u8>,
    ...
    public fun sign_bitcoin_proposal(
        ...
        assert!(proposal.psbt == last_psbt, ErrorPsbtExpired);
        ...
        verify_bitcoin_signature(new_psbt, &signature, &participant_public_key);
    ...
    fun verify_bitcoin_signature(psbt: address, signature: &vector<u8>, public_key: &vector<u8>) {
        assert!(
            ecdsa_k1::verify(
                signature,
                public_key,
    ...
    public fun get_proposal_status(multisign_address: address, proposal_id: u64): u8{
        assert!(multisign_account::is_multisign_account(multisign_address), ErrorMultisignAccountNotFound);
        let wallet = create_or_borrow_mut_wallet(multisign_address);
        assert!(table_vec::contains(&wallet.bitcoin_proposals, proposal_id), ErrorInvalidProposal);
        let proposal = table_vec::borrow_mut(&mut wallet.bitcoin_proposals, proposal_id);
    ...
    public fun get_current_no_signatures(multisign_address: address, proposal_id: u64): u64{
        assert!(multisign_account::is_multisign_account(multisign_address), ErrorMultisignAccountNotFound);
        let wallet = create_or_borrow_mut_wallet(multisign_address);
        assert!(table_vec::contains(&wallet.bitcoin_proposals, proposal_id), ErrorInvalidProposal);
        let proposal = table_vec::borrow_mut(&mut wallet.bitcoin_proposals, proposal_id);
